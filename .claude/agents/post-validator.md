...
- `tags`는 배열이고 비어있지 않음
- `published: true`
- **`title`·`description` 값에 "키워드: 요약" 콜론 구조 금지**: `title: "페이지 전환: 브라우저에 맡기기"` 같은 패턴은 AI 티 시그널. 자연스러운 구로 풀어쓰기. 예외는 백틱 안 식별자(``"`:has()` 셀렉터 실전 가이드"``)
  - 검출: frontmatter 영역에서 `^title:`·`^description:` 라인의 **따옴표 안 값**에 `: `(콜론+공백) 또는 영문자 뒤 `:`가 포함되면 에러. 값 전체가 백틱에 감싸여 있거나 콜론이 백틱 안에 갇혀 있으면 통과
...
- 함수명이 PascalCase인가?
- **사용 식별자와 import 선언 일치 (런타임 에러 방지)**: `code` prop 내부에서 `ReactDOM.xxx`, `ReactDOMClient.xxx`, 외부 라이브러리 네임스페이스(`lodash.`, `dayjs.` 등) 호출이 있으면 같은 code 블록 상단에 해당 import가 반드시 있어야 함. `React`(예: `React.useState`)만은 Sandpack `template="react"`가 자동 주입하므로 예외
  - 검출: 각 code prop 문자열 내부에서 정규식 `\b([A-Z][A-Za-z0-9]+)\.(\w+)`로 네임스페이스 호출을 수집 → `React` 제외 → 각 이름에 대해 `import (.+ from )?"[^"]*"` 블록에 해당 식별자가 존재하는지 확인
  - **특히 `ReactDOM.flushSync`는 확정 에러**: Sandpack react 템플릿이 `ReactDOM`을 자동 주입하지 않아 실제 런타임 에러가 납니다. `import { flushSync } from "react-dom";` 후 `flushSync(...)`로 바꾸라고 블로커로 지적
  - destructured import를 기본 원칙으로 권장. `ReactDOM.flushSync` → `flushSync` / `ReactDOMClient.createRoot` → `createRoot`
