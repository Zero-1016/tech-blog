...
- **사용한 식별자는 전부 import 선언**. Sandpack `template="react"`는 `React`만 자동 주입합니다. `ReactDOM`, `flushSync`, 외부 라이브러리는 전부 명시적 import 필요. 특히 네임스페이스 호출(`ReactDOM.flushSync(...)`)은 실제 Sandpack에서 런타임 에러를 냄
  - ❌ `document.startViewTransition(() => { ReactDOM.flushSync(() => setX(next)); });` (import 없음)
  - ✅ 파일 상단 `import { flushSync } from "react-dom";` 후 `flushSync(() => setX(next))`
  - **원칙**: `ReactDOM.xxx` / `ReactDOMClient.xxx` 네임스페이스 호출 금지. 항상 destructured import로 뽑아 쓰기. 외부 라이브러리도 동일
...
- **헤딩·title·description 콜론 구조 금지**: `## SPA: 한 줄이면 시작`, `title: "페이지 전환: 브라우저에 맡기는 법"` 같은 "키워드: 요약" 형태는 AI 티가 가장 강한 패턴 중 하나. 자연스러운 한국어 구로 풀어쓰기 (`## SPA에서 한 줄로 시작하기`, `title: "페이지 전환을 브라우저에 맡기는 법"`). 예외는 백틱 안의 CSS 의사요소·식별자(`` `::view-transition` ``, `` `:has()` `` 등)뿐
...
- **기본 어미는 구어 존댓말**: `~에요/~예요`, `~죠`, `~거든요`, `~고요`, `~어요`가 주력(전체 종결 어미의 70% 이상). `~입니다/~합니다`는 강조·전환·마무리에만 간간이(30% 이하). **반말(`~다`, `~이다`, `~한다`, `~된다`, `~있다`) 절대 금지** — 이건 블로그 글이지 교과서가 아님
- **실제 블로그 톤 예시** (기존 글에서 추출, 이 톤을 따라할 것):
...
- **호흡 다양화**: 짧은 문장과 긴 문장 섞기. 한 문단이 같은 종결 어미로 3번 이상 끝나면 안 됨
- **자기 목소리**: "처음엔 이게 왜 안 되나 싶었어요", "저도 그랬거든요" 같은 1인칭 경험 한두 줄. 독자에게 "~해보세요" 식 행동 유도도 자연스러움
- **구어 접속사 우선**: "그런데" → "근데", "하지만" → "다만", "따라서" → "그래서". 글말보다 입말 접속사
