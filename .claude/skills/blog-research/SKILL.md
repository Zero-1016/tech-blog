...
### Step 6: URL 도달성 최종 확인 (반환 직전 필수)

수집된 출처를 결과에 포함시키기 직전에 각 URL 의 200 OK 도달성을 한 번 더
확인합니다. 도메인이 §DOMAIN-PRIORITY-1 화이트리스트에 있다는 것만으로는
부족해요. 같은 도메인 안에서 경로가 잘못된 케이스 (404, redirect 루프, 경로
변경) 가 자료 수집 중 발생할 수 있거든요.

**검증 절차**:

1. **자동 통과**: Step 1~4 에서 WebFetch 로 본문을 직접 읽은 URL 은 자동 통과
   (200 응답으로 본문을 받았으니 도달성 확인됨)
2. **재확인 대상**: context7 응답에서만 추출된 URL, 추정 URL, 본문을 못 받은
   URL (redirect 루프 / 메타만 받은 케이스 등) → 명시적으로 한 번 WebFetch
   해서 200 확인
3. **실패 처리**: 4xx / 5xx / redirect 루프 응답이면:
   - WebSearch 로 정확한 URL 재탐색 (1회 시도, `<topic> site:<도메인>` 쿼리)
   - 찾으면 교체 후 결과에 포함
   - 못 찾으면 그 URL 은 결과에서 **제외**
4. **검증 통과한 URL 만** 출력 형식에 포함

**한도 처리**: WebFetch 한도 (6회) 가 이미 소진된 상태에서 재확인이 필요한
URL 이 남았다면, 해당 URL 을 "도달성 미확인" 으로 분류하고 결과의 수집 메모에
명시. 절대 도달성 미확인 URL 을 1순위로 포함하지 않음.

**특히 주의**: context7 결과는 redirect 루프나 메타 응답으로 본문을 직접 받지
못한 경우가 잦아요. context7 출처는 **항상 한 번 더 WebFetch 로 200 확인**
하는 게 안전합니다 (이번 정책의 핵심 동기).
...
- **URL 도달성 검증**: 통과 <P>개 / 교체 <Q>개 / 제외 <R>개 / 미확인 <S>개
...
- <URL 도달성 검증 특이 케이스 — 교체된 URL, 제외된 URL, redirect 루프 발생 등>
- 예: "이 주제는 Chrome/Safari 동작이 다름. 브라우저 지원 섹션에서 구분 필요."
- 예: "공식 스펙이 Working Draft 단계. 프로덕션 사용 주의 필요."
- 예: "context7 의 X 페이지가 redirect 루프 — Step 6 에서 정확한 경로 Y 로 교체"
```
