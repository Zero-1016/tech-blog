...
`dry_run: true` 가 입력으로 들어오면 모든 Phase 에서 다음을 준수:

- **Edit 툴 호출 금지** — 자동 수정도, 사용자 확인 후 수정도 전부 호출 안 함
- **velite 빌드는 실행하되 파일 수정 없음** — 결과만 보고
- **모든 발견 항목을 "수정 가능 분류" 로 정리해서 반환**

dry_run 이 false (기본) 면 기존 동작 그대로.
...
- `series_name`: 시리즈일 때
- `via`: `"orchestrator"` 또는 `"blog-revise"` (skip list 적용)
- `dry_run`: `true` 또는 `false` (기본 false). `true` 면 자동 수정 안 함, 발견 항목만 보고.
...
`dry_run: true` 일 때 validator 는 다음과 같이 동작:

- **자동 수정 안 함** — 확정 에러도 Edit 호출 없이 보고만
- **사용자 확인 항목도 Edit 안 함** — 보고 형식만 반환
- **검증 단계는 동일** — Phase 1~5 전부 실행
- **velite 빌드는 실행하되 결과만 보고** — 통과/실패 분리해서 알림

dry_run 의 목적: **진단 리포트 생성**. blog-revise 의 패턴 5 (분석만) 에서 사용.

dry_run 결과 형식:

```markdown
# validator dry_run 결과:
...
dry_run 이라 실제 수정은 안 됐습니다. 정상 모드로 다시 호출하면 자동 수정 가능한
N건이 처리됩니다.
```

오케스트레이터는 이 결과를 받아 사용자에게 표시하거나 추가 액션을 결정.
...
- 파일은 이 skill이 **Edit으로 직접 수정**합니다 (확정 에러만). 오케스트레이터가
  수정을 적용하지 않아요. **단, `dry_run: true` 면 Edit 호출 일체 금지** — 발견만
  하고 보고.
