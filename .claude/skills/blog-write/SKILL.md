...
if [ -f content/tmp/draft-feedback.md ]; then
  FEEDBACK_COUNT=$(grep -c '^## ' \
    content/tmp/draft-feedback.md)
  COMPLEXITY_COUNT=$(grep -c '^태그:.*complexity' \
    content/tmp/draft-feedback.md)
else
  FEEDBACK_COUNT=0
  COMPLEXITY_COUNT=0
...
`COMPLEXITY_COUNT` 가 3 이상이면 알림:


```
⚠️  GATE 1 복잡도 피드백 누적
   ${COMPLEXITY_COUNT}건 (전체 ${FEEDBACK_COUNT}건)

content/tmp/draft-feedback.md 에서
"기획안이 복잡하다" 패턴이 반복되고 있어요.

/blog-rule-editor 로 패턴을 분석한 뒤
blog-draft-review 의 D2 또는
blog-write Phase 3 가이드에
반영하는 걸 추천해요.
...
```
AskUserQuestion("이미 비슷한 주제의 글이 있어요. 어떻게 할까요?"):
- 계속 진행 — 다른 각도로 접근
- 취소 — 기존 글과 중복 방지
- 기존 글 수정 권장 — 내가 직접 편집
```
...
```
AskUserQuestion("자료 수집이 실패했어요. 어떻게 할까요?"):
...
blog-research 가 WebFetch 6회 / context7 3회 한도 넘긴 경우. 상황 설명 후 AskUserQuestion 호출:

```
AskUserQuestion("자료가 너무 분산되어 있어서 수집 한도에 걸렸어요. 어떻게 할까요?"):
- 주제를 하위 주제로 분할해서 재시도
- 주제 범위를 좁혀서 재시도
- 핵심 URL 을 직접 제공
- 취소
...
### Step 3-2: 가제(working title) 1개

**중요**: 최종 제목/설명은 **본문 작성 후 Phase 4.5 에서 본문 기반으로 다시 후보를
생성**합니다. 이 단계에서는 기획·메모용 가제 1개만 잡습니다.

가제 기준 (가벼움):

- 글이 다루는 주제 식별 가능
- slug 파생 가능
- §META-TITLE 의 엄격한 품질 바 (호기심 유발, 군더더기 제거 등) 는 **여기서는 안 봄**
- 콜론 구조·em-dash 같은 표면 금지만 지킴

설명도 가제 수준의 임시 한 문장. 50자 기준 안 맞아도 OK.

가제는 GATE 1 에서 사용자가 보고 "OK 수준이면 통과" 시키는 거라 부담을 낮춥니다.
...
### Step 3-6: 인터랙티브 데모 계획

`§MDX-DEMO-DENSITY` 기준으로 데모 밀집형 여부를 판단.

1. 주제 태그와 수집된 자료를 보고 4가지 조건 중 해당 개수 확인
2. 2개 이상이면 **데모 밀집형** 분류
3. 데모 밀집형이면 각 섹션에 `데모: <한 줄 설명>` 항목 추가

```markdown
데모 밀도: 밀집형 (CSS 속성 시각화 + 비교 데모 + 인터랙션)

- 섹션 1 "기본 동작": 데모 — 선택자 적용 전/후 비교
- 섹션 2 "실전 패턴": 데모 — 3가지 패턴을 탭으로 전환
- 섹션 3 "브라우저 차이": 데모 없음 (지원 표만)
- 섹션 4 "조합 활용": 데모 — 폼 유효성 상태에 따른 스타일 변화
```

일반형이면 데모 항목 생략하거나 핵심 섹션 1~2개에만 표시.
...
slug: <kebab-case (가제 기반, 본문 작성 후 변경 가능)>
예상 분량: 2000자 (약 4분 읽기)
데모 밀도: <밀집형|일반형|이론형> (<판단 근거 한 줄>)

## 가제 (working title)

<한 줄 가제>

> 최종 제목과 설명은 본문 작성 후 Phase 4.5 에서 본문 기반 후보 3개로 다시 잡습니다.
> 여기서는 OK 수준이면 통과시켜주세요.
...
```
AskUserQuestion("draft-review 가 시리즈 전환을 권장해요. 단편 자료가 많아서 한 글에 담기 어려울 수 있어요. 어떻게 할까요?"):
- 시리즈 (2편) 으로 전환 — 기획 재작성
- 단편 유지하되 자료 일부 정리
- 주제 축소해서 단편 유지
...
```
AskUserQuestion("draft-review 가 블로커 결함을 발견했어요: <문제 요약>. 어떻게 할까요?"):
...
```
AskUserQuestion("기획안 검토 — 어떻게 진행할까요?"):
- 진행 — 이 기획안으로 글 작성 시작
- 예시 첨부 — 글에 넣을 스크린샷·코드·데모가 있어요
...
- **B (예시 첨부)**: 사용자에게 예시 소재를 받음 (스크린샷 경로, 코드 스니펫, URL,
  터미널 출력 등). 받은 소재를 기획안의 해당 섹션에 "사용자 제공 예시" 로 추가하고
  GATE 1 재제시. writer 에게 전달할 때 `user_examples` 필드로 함께 넘김.
  writer 는 이 소재를 본문의 자연스러운 위치에 코드 블록, 이미지, 또는 인라인
  예시로 녹여야 함.
- **C (수정)**: 사용자가 어느 부분 수정할지 알려줌. 해당 부분 수정 후 GATE 1 재제시. 최대 3회. **자동 로그 기록**: Step GATE-1-LOG.
- **D (재검토)**: Phase 2 (자료 수집) 부터 재시작. `user_urls` 추가 가능. **자동 로그 기록**: Step GATE-1-LOG.
- **E (취소)**: 종료. `content/tmp/draft-feedback.md` 만 기록 (취소 사유 분석용). **자동 로그 기록**: Step GATE-1-LOG.

**C가 3회 반복되면**: "기획안에 근본 문제가 있는 것 같아요. D (주제 재검토) 를 추천해요." 알림.

### Step GATE-1-LOG: draft-feedback.md 자동 기록

C/D/E 응답 직후 실행. 사용자 자유 텍스트:
...
echo "$USER_TEXT" \
  | grep -qiE "복잡|단순|길|많|광범위|어렵|줄여|축소|쪼개|좁혀" \
  && TAGS="${TAGS} complexity"

cat >> content/tmp/draft-feedback.md <<EOF

## ${TS} — <plan_title>

응답 종류: <C 수정 / D 재검토 / E 취소>
시도: <N>/3 (C 일 때만)
태그:${TAGS:- (없음)}

기획안 메타:
- 제목: <plan_title>
- 복잡도 라벨: <single | series | unknown>
- 섹션 수: <H2 개수>
- 자료 수: <sources 길이>
...
- 분류·해석 안 함. 자유 텍스트 그대로.
- 키워드 태깅은 검색 보조용. 자동 분류 X.
- A (진행), B (예시 첨부) 는 기록 안 함.
- E (취소) 는 기록 (사유 패턴 분석용).
...
user_examples: <GATE 1 에서 사용자가 제공한 예시 소재 배열 (선택적)>
today: <YYYY-MM-DD>
...
경로를 `saved_files` 에 저장하고 **Phase 4.5 (제목/설명 확정)** 진입.
...
## Phase 4.5: 제목/설명 확정 (GATE 2)

본문이 작성됐으니, 이제 본문을 기반으로 최종 제목과 설명을 확정한다. Phase 3 의
가제는 임시값이었고, 여기서 §META-TITLE / §META-DESCRIPTION 의 엄격한 품질 바를
적용한다.

### Step 4.5-1: 본문 통독

저장된 파일을 Read 로 읽고 본문 전체 파악:

```

Read content/posts/<slug>.mdx
...
- 도입부의 핵심 의문/문제
- 본문에서 다뤄진 구체적 결과·수치·예시
- 마무리의 결론 한 문장

### Step 4.5-2: 제목 후보 3개 생성 (§META-TITLE 적용)

본문 신호를 바탕으로 후보 3개:

- 후보 1: 호기심 유발형 ("왜 ~할까")
- 후보 2: 결과·체감형 ("~만 잡아도 ~돌아온다")
- 후보 3: 핵심 식별자형 (글의 핵심 기술명을 자연스럽게 노출)

각 후보가 §META-TITLE 의 모든 기준 통과 (20자 이내, 콜론·em-dash 금지, 반말 문어체 금지 등) 확인.

### Step 4.5-3: 설명 후보 3개 생성 (§META-DESCRIPTION 적용)

본문의 "이 글이 풀어주는 문제" 한 문장으로 후보 3개:

- 후보 1: 독자 장면 중심 ("~할 때 막히는 지점을 정리합니다")
- 후보 2: 결론 미리보기 ("~의 정답은 ~예요")
- 후보 3: 비교·대비 ("~와 ~ 사이의 진짜 차이")

각 후보 50자 내외, 존댓말, 본문 톤과 일치.

### Step 4.5-4: GATE 2 (제목 선택)

```

## 제목 후보

본문을 다 쓰고 나서 본문 기반으로 다시 잡았어요. 어느 게 좋을까요?
...
AskUserQuestion("최종 제목을 선택해주세요"):

- 후보 1: <후보 1 텍스트>
- 후보 2: <후보 2 텍스트>
- 후보 3: <후보 3 텍스트>
- 직접 입력 (다음 메시지로 새 제목 보내기)
- 후보 다시 — 다른 톤으로 3개 더

```

"직접 입력" 선택 시 사용자 응답 대기 후 그 값 사용. "후보 다시" 선택 시 Step 4.5-2 재실행.

### Step 4.5-5: GATE 2 (설명 선택)

제목과 동일한 패턴으로 AskUserQuestion 호출:

```

AskUserQuestion("최종 설명을 선택해주세요"):
...
선택된 최종 제목·설명으로 frontmatter 의 `title:` 과 `description:` 라인을 Edit:

```

Edit(
file_path="content/posts/<slug>.mdx",
old_string='title: "<가제>"\ndescription: "<임시 설명>"',
new_string='title: "<최종 제목>"\ndescription: "<최종 설명>"'
)

```

시리즈일 때는 편 제목 (`title`) 만 갱신. 시리즈 제목 (`series`) 은 Phase 3 에서
정한 그대로 (시리즈 제목은 본문에 덜 의존하니까).

### Step 4.5-7: slug 변경 검토 (선택)

가제와 최종 제목이 크게 달라서 기존 slug 가 어색해진 경우만:

```

AskUserQuestion("slug 도 바꿀까요? (URL 영향 있음)"):
...
새 slug 선택 시: 파일명 변경 (`mv` 또는 Read → Write 새 파일 + rm 옛 파일) +
frontmatter 의 어떤 slug 필드도 자동 동기화. 이때 기존 글이 이 slug 를 internal
link 로 참조하고 있는지 grep 으로 확인 후 사용자 알림.

**기본은 slug 유지**. 묻지 않고 넘어가는 게 디폴트 (한 번 더 물으면 짜증).

### 시리즈 모드 특수 처리

시리즈일 때:

- 시리즈 제목 (`series` 필드): Phase 3 가제 그대로. Phase 4.5 에서 갱신 안 함
  (시리즈 통일성 우선)
- 편 제목 (`title` 필드): 각 편마다 본문 기반으로 후보 3개 생성 → GATE 2 진행
- 편이 여러 개면 각 편마다 Step 4.5-1~6 반복

### 완료 후

`saved_files` 의 frontmatter 가 최종 값으로 갱신된 상태로 Phase 5 (validator) 진입.

validator 는 §META-TITLE / §META-DESCRIPTION 검사를 다시 한 번 돌리지만,
Phase 4.5 에서 이미 §META-TITLE 적용했으니 통과해야 정상.
...
```
AskUserQuestion("writer 가 규칙 위반 없는 초안을 만드는 데 실패했어요. 어떻게 복구할까요?"):
...
```
AskUserQuestion("writer 실패가 누적 TOTAL건이에요. 장기 개선과 이번 글 복구를 어떻게 할까요?"):
...
```
AskUserQuestion("L5 title 콜론 구조 — 어느 버전으로 재작성할까요?"):
...
2회 재시도 후에도 실패하면 사용자 에스컬레이션. 상황 설명 후 AskUserQuestion 호출:

```
AskUserQuestion("writer 재실행을 2회 시도했지만 validator 에러가 계속 나와요. 어떻게 할까요?"):
- 이번 글 취소 (저장된 파일 삭제)
...
validator 가 자동 수정 안 함. 에러 원문을 텍스트로 제시하고 AskUserQuestion 호출:
...
```
AskUserQuestion("velite 스키마 에러 — 어떻게 할까요?"):
...
```
AskUserQuestion("L34 병렬 삼단 — 어느 제안으로 수정할까요?"):
...
```
AskUserQuestion("도입-결론 불일치 (E1) — 마무리에 오프닝 답변을 어떻게 추가할까요?"):
...
- **content/tmp/writer-failures.md 와 content/tmp/draft-feedback.md 는 덮어쓰지 말고 append**.
