...
- `§RULE-TERM-INTRODUCTION` — 약어·기술 용어 첫 등장 시 풀이
...
- 약어·기술 용어 첫 등장 풀이 (§RULE-TERM-INTRODUCTION, 글 전체 통독 후 판단)
...
### 8-13. 약어·기술 용어 도입 (§RULE-TERM-INTRODUCTION)

본문에서 약어 (PWA, CDN, SSR 등), 이벤트 이름 (`install`, `activate`, `fetch`,
`sync` 등), 라이프사이클 단계 이름 (`installing`, `activating` 등), API 메서드
이름 (`skipWaiting()`, `respondWith()` 등) 이 처음 등장할 때 풀어쓰기 / 한 줄
정의 / 인라인 링크 중 하나가 함께 있는지 확인.

결론·요약 단락 (마지막 H2 또는 마무리 단락) 에서 등장하는 용어가 본문에서
한 번이라도 풀이됐는지 확인. 본문에 한 번도 풀이되지 않은 용어를 마무리에서
던지면 위반.

§RULE-TERM-INTRODUCTION 의 예외에 해당하면 생략 가능:

- 글의 핵심 식별자 (글 자체가 그 용어를 다룸. 예: Service Worker 글에서
  Service Worker)
- 대상 독자가 통상 아는 약어 (frontmatter `tags` 와 주제 범위로 판단. React/
  Next.js 글에서 SSR / CSR / SSG / ISR / RSC 등)
- 헤딩 등장 용어는 직전 본문에 사전 설명 한 줄 있으면 통과
