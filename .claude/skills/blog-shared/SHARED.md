...
5. **frontmatter의 `title`, `description`, `tags`** — YAML 문자열
6. **JSX 컴포넌트의 모든 prop 값** (문자열, 배열 안 객체의 값, 객체 prop)
...
- 단편: `/posts/<slug>`
- 시리즈: `/series/<seriesSlug>` 또는 `/posts/<seriesSlug>/<partSlug>`
...
- `/posts/<slug>` → `content/posts/<slug>.mdx` 또는 `content/posts/*/<slug>.mdx` Glob 매칭
- `/series/<seriesSlug>` → `content/posts/<seriesSlug>/` 디렉토리 존재 + 편들의 frontmatter
  `series` 값 일치

**검출 전략** (grep):

1. `\]\((?!/posts/|/series/|https?://|#)` — 접두사 위반
...
- 이론/개념형 (렌더링 파이프라인, 알고리즘 설명 등): CodePlayground 0~1개. AnimatedStep이나 다이어그램이 더 적합
